...
usage(OptSpecList) ->
  getopt:usage(OptSpecList, "kucumberl", "", []).
...
store_conf(Conf, [I | Rest]) ->
  case I of
    ...
    {format, F} -> NewConf = Conf#conf{format = list_to_atom(F)};
      ...
      case kucumberl_parser:parse(File) of
        {ok, F} ->
          F1 = case Conf#conf.tags of
            [[]] -> F#feature{id = F#feature.name};
            Tags ->
              disable_not_tagged_scenarios(F#feature{id = F#feature.name}, Tags)
          end,
          NextID = ID + 1,
